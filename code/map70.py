...
locations70_raw = pd.read_csv('../locations/locations_70.csv', parse_dates=['datetime'])

print("=== Client ID 70 Dataset ===")

# clean data - drop rows with missing values
locations70_clean = locations70_raw.dropna()

# filter data - drop rows with low accuracy
locations70 = locations70_clean[locations70_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations70['month_year'] = locations70['datetime'].dt.to_period('M')
...
locations70['int_latitude'] = locations70['latitude'].astype(int)
locations70['int_longitude'] = locations70['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations70.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations70.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations70["time_bucket"] = locations70["datetime"].dt.floor("30min")
locations70_buckets = locations70.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations70_buckets.columns:
    locations70_buckets["client_id"] = locations70["client_id"].iloc[0]

# calculate time spent at each bucket location
locations70_buckets = locations70_buckets.sort_values("datetime")
locations70_buckets["next_time"] = locations70_buckets["datetime"].shift(-1)
locations70_buckets["time_spent"] = (locations70_buckets["next_time"] - locations70_buckets["datetime"]).dt.total_seconds()
locations70_buckets["time_spent"] = locations70_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations70_buckets["x"], locations70_buckets["y"] = proj.transform(locations70_buckets["longitude"].values, locations70_buckets["latitude"].values)

# clustering per month
locations70_buckets["month"] = locations70_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations70_buckets.groupby("month"):
...
# combine
locations70_buckets = pd.concat(df_list)
...
locations70_buckets = merge_overlapping_clusters(locations70_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations70_buckets["cluster_uid"] = locations70_buckets["cluster_uid_merged"]
locations70_buckets = locations70_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations70_buckets[locations70_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations70_buckets[locations70_buckets["cluster_id"] != -1]
...
locations70_buckets["hour"] = locations70_buckets["datetime"].dt.hour
locations70_buckets["weekday"] = locations70_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations70_buckets["time_category"] = locations70_buckets.apply(categorize, axis=1)

valid = locations70_buckets[locations70_buckets["cluster_id"] != -1]
...
df_heat = locations70_buckets[locations70_buckets["cluster_id"] != -1].copy()
...
df_seq = locations70_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap70.html")
print("heatmap saved → heatmap70.html")