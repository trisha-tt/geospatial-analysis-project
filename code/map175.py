...
locations175_raw = pd.read_csv('../locations/locations_175.csv', parse_dates=['datetime'])

print("=== Client ID 175 Dataset ===")

# clean data - drop rows with missing values
locations175_clean = locations175_raw.dropna()

# filter data - drop rows with low accuracy
locations175 = locations175_clean[locations175_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations175['month_year'] = locations175['datetime'].dt.to_period('M')
...
locations175['int_latitude'] = locations175['latitude'].astype(int)
locations175['int_longitude'] = locations175['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations175.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations175.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations175["time_bucket"] = locations175["datetime"].dt.floor("30min")
locations175_buckets = locations175.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations175_buckets.columns:
    locations175_buckets["client_id"] = locations175["client_id"].iloc[0]

# calculate time spent at each bucket location
locations175_buckets = locations175_buckets.sort_values("datetime")
locations175_buckets["next_time"] = locations175_buckets["datetime"].shift(-1)
locations175_buckets["time_spent"] = (locations175_buckets["next_time"] - locations175_buckets["datetime"]).dt.total_seconds()
locations175_buckets["time_spent"] = locations175_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations175_buckets["x"], locations175_buckets["y"] = proj.transform(locations175_buckets["longitude"].values, locations175_buckets["latitude"].values)

# clustering per month
locations175_buckets["month"] = locations175_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations175_buckets.groupby("month"):
...
# combine
locations175_buckets = pd.concat(df_list)
...
locations175_buckets = merge_overlapping_clusters(locations175_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations175_buckets["cluster_uid"] = locations175_buckets["cluster_uid_merged"]
locations175_buckets = locations175_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations175_buckets[locations175_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations175_buckets[locations175_buckets["cluster_id"] != -1]
...
locations175_buckets["hour"] = locations175_buckets["datetime"].dt.hour
locations175_buckets["weekday"] = locations175_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations175_buckets["time_category"] = locations175_buckets.apply(categorize, axis=1)

valid = locations175_buckets[locations175_buckets["cluster_id"] != -1]
...
df_heat = locations175_buckets[locations175_buckets["cluster_id"] != -1].copy()
...
df_seq = locations175_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap175.html")
print("heatmap saved → heatmap175.html")