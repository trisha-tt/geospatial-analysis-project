...
locations177_raw = pd.read_csv('../locations/locations_177.csv', parse_dates=['datetime'])

print("=== Client ID 177 Dataset ===")

# clean data - drop rows with missing values
locations177_clean = locations177_raw.dropna()

# filter data - drop rows with low accuracy
locations177 = locations177_clean[locations177_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations177['month_year'] = locations177['datetime'].dt.to_period('M')
...
locations177['int_latitude'] = locations177['latitude'].astype(int)
locations177['int_longitude'] = locations177['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations177.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations177.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations177["time_bucket"] = locations177["datetime"].dt.floor("30min")
locations177_buckets = locations177.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations177_buckets.columns:
    locations177_buckets["client_id"] = locations177["client_id"].iloc[0]

# calculate time spent at each bucket location
locations177_buckets = locations177_buckets.sort_values("datetime")
locations177_buckets["next_time"] = locations177_buckets["datetime"].shift(-1)
locations177_buckets["time_spent"] = (locations177_buckets["next_time"] - locations177_buckets["datetime"]).dt.total_seconds()
locations177_buckets["time_spent"] = locations177_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations177_buckets["x"], locations177_buckets["y"] = proj.transform(locations177_buckets["longitude"].values, locations177_buckets["latitude"].values)

# clustering per month
locations177_buckets["month"] = locations177_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations177_buckets.groupby("month"):
...
# combine
locations177_buckets = pd.concat(df_list)
...
locations177_buckets = merge_overlapping_clusters(locations177_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations177_buckets["cluster_uid"] = locations177_buckets["cluster_uid_merged"]
locations177_buckets = locations177_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations177_buckets[locations177_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations177_buckets[locations177_buckets["cluster_id"] != -1]
...
locations177_buckets["hour"] = locations177_buckets["datetime"].dt.hour
locations177_buckets["weekday"] = locations177_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations177_buckets["time_category"] = locations177_buckets.apply(categorize, axis=1)

valid = locations177_buckets[locations177_buckets["cluster_id"] != -1]
...
df_heat = locations177_buckets[locations177_buckets["cluster_id"] != -1].copy()
...
df_seq = locations177_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap177.html")
print("heatmap saved → heatmap177.html")