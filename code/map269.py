...
locations269_raw = pd.read_csv('../locations/locations_269.csv', parse_dates=['datetime'])

print("=== Client ID 269 Dataset ===")

# clean data - drop rows with missing values
locations269_clean = locations269_raw.dropna()

# filter data - drop rows with low accuracy
locations269 = locations269_clean[locations269_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations269['month_year'] = locations269['datetime'].dt.to_period('M')
...
locations269['int_latitude'] = locations269['latitude'].astype(int)
locations269['int_longitude'] = locations269['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations269.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations269.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations269["time_bucket"] = locations269["datetime"].dt.floor("30min")
locations269_buckets = locations269.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations269_buckets.columns:
    locations269_buckets["client_id"] = locations269["client_id"].iloc[0]

# calculate time spent at each bucket location
locations269_buckets = locations269_buckets.sort_values("datetime")
locations269_buckets["next_time"] = locations269_buckets["datetime"].shift(-1)
locations269_buckets["time_spent"] = (locations269_buckets["next_time"] - locations269_buckets["datetime"]).dt.total_seconds()
locations269_buckets["time_spent"] = locations269_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations269_buckets["x"], locations269_buckets["y"] = proj.transform(locations269_buckets["longitude"].values, locations269_buckets["latitude"].values)

# clustering per month
locations269_buckets["month"] = locations269_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations269_buckets.groupby("month"):
...
# combine
locations269_buckets = pd.concat(df_list)
...
locations269_buckets = merge_overlapping_clusters(locations269_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations269_buckets["cluster_uid"] = locations269_buckets["cluster_uid_merged"]
locations269_buckets = locations269_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations269_buckets[locations269_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations269_buckets[locations269_buckets["cluster_id"] != -1]
...
locations269_buckets["hour"] = locations269_buckets["datetime"].dt.hour
locations269_buckets["weekday"] = locations269_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations269_buckets["time_category"] = locations269_buckets.apply(categorize, axis=1)

valid = locations269_buckets[locations269_buckets["cluster_id"] != -1]
...
df_heat = locations269_buckets[locations269_buckets["cluster_id"] != -1].copy()
...
df_seq = locations269_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap269.html")
print("heatmap saved → heatmap269.html")