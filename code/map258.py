...
locations258_raw = pd.read_csv('../locations/locations_258.csv', parse_dates=['datetime'])

print("=== Client ID 258 Dataset ===")

# clean data - drop rows with missing values
locations258_clean = locations258_raw.dropna()

# filter data - drop rows with low accuracy
locations258 = locations258_clean[locations258_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations258['month_year'] = locations258['datetime'].dt.to_period('M')
...
locations258['int_latitude'] = locations258['latitude'].astype(int)
locations258['int_longitude'] = locations258['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations258.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations258.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations258["time_bucket"] = locations258["datetime"].dt.floor("30min")
locations258_buckets = locations258.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations258_buckets.columns:
    locations258_buckets["client_id"] = locations258["client_id"].iloc[0]

# calculate time spent at each bucket location
locations258_buckets = locations258_buckets.sort_values("datetime")
locations258_buckets["next_time"] = locations258_buckets["datetime"].shift(-1)
locations258_buckets["time_spent"] = (locations258_buckets["next_time"] - locations258_buckets["datetime"]).dt.total_seconds()
locations258_buckets["time_spent"] = locations258_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations258_buckets["x"], locations258_buckets["y"] = proj.transform(locations258_buckets["longitude"].values, locations258_buckets["latitude"].values)

# clustering per month
locations258_buckets["month"] = locations258_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations258_buckets.groupby("month"):
...
# combine
locations258_buckets = pd.concat(df_list)
...
locations258_buckets = merge_overlapping_clusters(locations258_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations258_buckets["cluster_uid"] = locations258_buckets["cluster_uid_merged"]
locations258_buckets = locations258_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations258_buckets[locations258_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations258_buckets[locations258_buckets["cluster_id"] != -1]
...
locations258_buckets["hour"] = locations258_buckets["datetime"].dt.hour
locations258_buckets["weekday"] = locations258_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations258_buckets["time_category"] = locations258_buckets.apply(categorize, axis=1)

valid = locations258_buckets[locations258_buckets["cluster_id"] != -1]
...
df_heat = locations258_buckets[locations258_buckets["cluster_id"] != -1].copy()
...
df_seq = locations258_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap258.html")
print("heatmap saved → heatmap258.html")