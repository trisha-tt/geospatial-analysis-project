...
locations179_raw = pd.read_csv('../locations/locations_179.csv', parse_dates=['datetime'])

print("=== Client ID 179 Dataset ===")

# clean data - drop rows with missing values
locations179_clean = locations179_raw.dropna()

# filter data - drop rows with low accuracy
locations179 = locations179_clean[locations179_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations179['month_year'] = locations179['datetime'].dt.to_period('M')
...
locations179['int_latitude'] = locations179['latitude'].astype(int)
locations179['int_longitude'] = locations179['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations179.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations179.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations179["time_bucket"] = locations179["datetime"].dt.floor("30min")
locations179_buckets = locations179.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations179_buckets.columns:
    locations179_buckets["client_id"] = locations179["client_id"].iloc[0]

# calculate time spent at each bucket location
locations179_buckets = locations179_buckets.sort_values("datetime")
locations179_buckets["next_time"] = locations179_buckets["datetime"].shift(-1)
locations179_buckets["time_spent"] = (locations179_buckets["next_time"] - locations179_buckets["datetime"]).dt.total_seconds()
locations179_buckets["time_spent"] = locations179_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations179_buckets["x"], locations179_buckets["y"] = proj.transform(locations179_buckets["longitude"].values, locations179_buckets["latitude"].values)

# clustering per month
locations179_buckets["month"] = locations179_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations179_buckets.groupby("month"):
...
# combine
locations179_buckets = pd.concat(df_list)
...
locations179_buckets = merge_overlapping_clusters(locations179_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations179_buckets["cluster_uid"] = locations179_buckets["cluster_uid_merged"]
locations179_buckets = locations179_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations179_buckets[locations179_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations179_buckets[locations179_buckets["cluster_id"] != -1]
...
locations179_buckets["hour"] = locations179_buckets["datetime"].dt.hour
locations179_buckets["weekday"] = locations179_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations179_buckets["time_category"] = locations179_buckets.apply(categorize, axis=1)

valid = locations179_buckets[locations179_buckets["cluster_id"] != -1]
...
df_heat = locations179_buckets[locations179_buckets["cluster_id"] != -1].copy()
...
df_seq = locations179_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap179.html")
print("heatmap saved → heatmap179.html")