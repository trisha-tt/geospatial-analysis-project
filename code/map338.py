...
locations338_raw = pd.read_csv('../locations/locations_338.csv', parse_dates=['datetime'])

print("=== Client ID 338 Dataset ===")

# clean data - drop rows with missing values
locations338_clean = locations338_raw.dropna()

# filter data - drop rows with low accuracy
locations338 = locations338_clean[locations338_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations338['month_year'] = locations338['datetime'].dt.to_period('M')
...
locations338['int_latitude'] = locations338['latitude'].astype(int)
locations338['int_longitude'] = locations338['longitude'].astype(int)
...
locations338["time_bucket"] = locations338["datetime"].dt.floor("30min")
locations338_buckets = locations338.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations338_buckets.columns:
    locations338_buckets["client_id"] = locations338["client_id"].iloc[0]

# calculate time spent at each bucket location
locations338_buckets = locations338_buckets.sort_values("datetime")
locations338_buckets["next_time"] = locations338_buckets["datetime"].shift(-1)
locations338_buckets["time_spent"] = (locations338_buckets["next_time"] - locations338_buckets["datetime"]).dt.total_seconds()
locations338_buckets["time_spent"] = locations338_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations338_buckets["x"], locations338_buckets["y"] = proj.transform(locations338_buckets["longitude"].values, locations338_buckets["latitude"].values)

# clustering per month
locations338_buckets["month"] = locations338_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations338_buckets.groupby("month"):
...
# combine
locations338_buckets = pd.concat(df_list)
...
locations338_buckets = merge_overlapping_clusters(locations338_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations338_buckets["cluster_uid"] = locations338_buckets["cluster_uid_merged"]
locations338_buckets = locations338_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations338_buckets[locations338_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations338_buckets[locations338_buckets["cluster_id"] != -1]
...
locations338_buckets["hour"] = locations338_buckets["datetime"].dt.hour
locations338_buckets["weekday"] = locations338_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations338_buckets["time_category"] = locations338_buckets.apply(categorize, axis=1)

valid = locations338_buckets[locations338_buckets["cluster_id"] != -1]
...
m2.save("./maps/heatmap338.html")
print("heatmap saved → heatmap338.html")