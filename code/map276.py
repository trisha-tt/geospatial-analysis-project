...
locations276_raw = pd.read_csv('../locations/locations_276.csv', parse_dates=['datetime'])

print("=== Client ID 276 Dataset ===")

# clean data - drop rows with missing values
locations276_clean = locations276_raw.dropna()

# filter data - drop rows with low accuracy
locations276 = locations276_clean[locations276_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations276['month_year'] = locations276['datetime'].dt.to_period('M')
...
locations276['int_latitude'] = locations276['latitude'].astype(int)
locations276['int_longitude'] = locations276['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations276.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations276.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations276["time_bucket"] = locations276["datetime"].dt.floor("30min")
locations276_buckets = locations276.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations276_buckets.columns:
    locations276_buckets["client_id"] = locations276["client_id"].iloc[0]

# calculate time spent at each bucket location
locations276_buckets = locations276_buckets.sort_values("datetime")
locations276_buckets["next_time"] = locations276_buckets["datetime"].shift(-1)
locations276_buckets["time_spent"] = (locations276_buckets["next_time"] - locations276_buckets["datetime"]).dt.total_seconds()
locations276_buckets["time_spent"] = locations276_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations276_buckets["x"], locations276_buckets["y"] = proj.transform(locations276_buckets["longitude"].values, locations276_buckets["latitude"].values)

# clustering per month
locations276_buckets["month"] = locations276_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations276_buckets.groupby("month"):
...
# combine
locations276_buckets = pd.concat(df_list)
...
locations276_buckets = merge_overlapping_clusters(locations276_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations276_buckets["cluster_uid"] = locations276_buckets["cluster_uid_merged"]
locations276_buckets = locations276_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations276_buckets[locations276_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations276_buckets[locations276_buckets["cluster_id"] != -1]
...
locations276_buckets["hour"] = locations276_buckets["datetime"].dt.hour
locations276_buckets["weekday"] = locations276_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations276_buckets["time_category"] = locations276_buckets.apply(categorize, axis=1)

valid = locations276_buckets[locations276_buckets["cluster_id"] != -1]
...
df_heat = locations276_buckets[locations276_buckets["cluster_id"] != -1].copy()
...
df_seq = locations276_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap276.html")
print("heatmap saved → heatmap276.html")