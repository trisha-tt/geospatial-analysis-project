...
locations328_raw = pd.read_csv('../locations/locations_328.csv', parse_dates=['datetime'])

print("=== Client ID 328 Dataset ===")

# clean data - drop rows with missing values
locations328_clean = locations328_raw.dropna()

# filter data - drop rows with low accuracy
locations328 = locations328_clean[locations328_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations328['month_year'] = locations328['datetime'].dt.to_period('M')
...
locations328['int_latitude'] = locations328['latitude'].astype(int)
locations328['int_longitude'] = locations328['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations328.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations328.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations328["time_bucket"] = locations328["datetime"].dt.floor("30min")
locations328_buckets = locations328.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations328_buckets.columns:
    locations328_buckets["client_id"] = locations328["client_id"].iloc[0]

# calculate time spent at each bucket location
locations328_buckets = locations328_buckets.sort_values("datetime")
locations328_buckets["next_time"] = locations328_buckets["datetime"].shift(-1)
locations328_buckets["time_spent"] = (locations328_buckets["next_time"] - locations328_buckets["datetime"]).dt.total_seconds()
locations328_buckets["time_spent"] = locations328_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations328_buckets["x"], locations328_buckets["y"] = proj.transform(locations328_buckets["longitude"].values, locations328_buckets["latitude"].values)

# clustering per month
locations328_buckets["month"] = locations328_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations328_buckets.groupby("month"):
...
# combine
locations328_buckets = pd.concat(df_list)
...
locations328_buckets = merge_overlapping_clusters(locations328_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations328_buckets["cluster_uid"] = locations328_buckets["cluster_uid_merged"]
locations328_buckets = locations328_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations328_buckets[locations328_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations328_buckets[locations328_buckets["cluster_id"] != -1]
...
locations328_buckets["hour"] = locations328_buckets["datetime"].dt.hour
locations328_buckets["weekday"] = locations328_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations328_buckets["time_category"] = locations328_buckets.apply(categorize, axis=1)

valid = locations328_buckets[locations328_buckets["cluster_id"] != -1]
...
df_heat = locations328_buckets[locations328_buckets["cluster_id"] != -1].copy()
...
df_seq = locations328_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap328.html")
print("heatmap saved → heatmap328.html")