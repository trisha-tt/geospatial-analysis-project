...
locations336_raw = pd.read_csv('../locations/locations_336.csv', parse_dates=['datetime'])

print("=== Client ID 336 Dataset ===")

# clean data - drop rows with missing values
locations336_clean = locations336_raw.dropna()

# filter data - drop rows with low accuracy
locations336 = locations336_clean[locations336_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations336['month_year'] = locations336['datetime'].dt.to_period('M')
...
locations336['int_latitude'] = locations336['latitude'].astype(int)
locations336['int_longitude'] = locations336['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations336.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations336.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations336["time_bucket"] = locations336["datetime"].dt.floor("30min")
locations336_buckets = locations336.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations336_buckets.columns:
    locations336_buckets["client_id"] = locations336["client_id"].iloc[0]

# calculate time spent at each bucket location
locations336_buckets = locations336_buckets.sort_values("datetime")
locations336_buckets["next_time"] = locations336_buckets["datetime"].shift(-1)
locations336_buckets["time_spent"] = (locations336_buckets["next_time"] - locations336_buckets["datetime"]).dt.total_seconds()
locations336_buckets["time_spent"] = locations336_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations336_buckets["x"], locations336_buckets["y"] = proj.transform(locations336_buckets["longitude"].values, locations336_buckets["latitude"].values)

# clustering per month
locations336_buckets["month"] = locations336_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations336_buckets.groupby("month"):
...
# combine
locations336_buckets = pd.concat(df_list)
...
locations336_buckets = merge_overlapping_clusters(locations336_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations336_buckets["cluster_uid"] = locations336_buckets["cluster_uid_merged"]
locations336_buckets = locations336_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations336_buckets[locations336_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations336_buckets[locations336_buckets["cluster_id"] != -1]
...
locations336_buckets["hour"] = locations336_buckets["datetime"].dt.hour
locations336_buckets["weekday"] = locations336_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations336_buckets["time_category"] = locations336_buckets.apply(categorize, axis=1)

valid = locations336_buckets[locations336_buckets["cluster_id"] != -1]
...
df_heat = locations336_buckets[locations336_buckets["cluster_id"] != -1].copy()
...
df_seq = locations336_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap336.html")
print("heatmap saved → heatmap336.html")