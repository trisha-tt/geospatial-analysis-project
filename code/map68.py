...
locations68_raw = pd.read_csv('../locations/locations_68.csv', parse_dates=['datetime'])

print("=== Client ID 68 Dataset ===")

# clean data - drop rows with missing values
locations68_clean = locations68_raw.dropna()

# filter data - drop rows with low accuracy
locations68 = locations68_clean[locations68_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations68['month_year'] = locations68['datetime'].dt.to_period('M')
...
locations68['int_latitude'] = locations68['latitude'].astype(int)
locations68['int_longitude'] = locations68['longitude'].astype(int)
...
locations68["time_bucket"] = locations68["datetime"].dt.floor("30min")
locations68_buckets = locations68.groupby("time_bucket").first().reset_index()
...
locations68_buckets["next_time"] = locations68_buckets["datetime"].shift(-1)
locations68_buckets["time_spent"] = (locations68_buckets["next_time"] - locations68_buckets["datetime"]).dt.total_seconds()
locations68_buckets["time_spent"] = locations68_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations68_buckets["x"], locations68_buckets["y"] = proj.transform(locations68_buckets["longitude"].values, locations68_buckets["latitude"].values)

# clustering per month
locations68_buckets["month"] = locations68_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations68_buckets.groupby("month"):
...
monthly = (
    locations68_buckets[locations68_buckets["cluster_id"] != -1]
...
locations68_buckets["hour"] = locations68_buckets["datetime"].dt.hour
locations68_buckets["weekday"] = locations68_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations68_buckets["time_category"] = locations68_buckets.apply(categorize, axis=1)

valid = locations68_buckets[locations68_buckets["cluster_id"] != -1]
...
df_heat = locations68_buckets[locations68_buckets["cluster_id"] != -1].copy()
...
df_seq = locations68_buckets.sort_values("datetime")
