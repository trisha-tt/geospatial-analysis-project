...
locations343_raw = pd.read_csv('../locations/locations_343.csv', parse_dates=['datetime'])

print("=== Client ID 343 Dataset ===")

# clean data - drop rows with missing values
locations343_clean = locations343_raw.dropna()

# filter data - drop rows with low accuracy
locations343 = locations343_clean[locations343_clean['accuracy'] < 1500].copy()

# convert date and time to month and year for filtering
locations343['month_year'] = locations343['datetime'].dt.to_period('M')
...
locations343['int_latitude'] = locations343['latitude'].astype(int)
locations343['int_longitude'] = locations343['longitude'].astype(int)

# first, we can find out how many locations a person visited in a month
monthly_count = locations343.groupby('month_year').size().reset_index(name='count')
# print("\n==Number of Locations visited in a Month==\n")
# print(monthly_count)

# find out how many times a person visits a location in a month
location_counts = locations343.groupby(['month_year', 'int_latitude', 'int_longitude']).size().reset_index(name='count')
...
locations343["time_bucket"] = locations343["datetime"].dt.floor("30min")
locations343_buckets = locations343.groupby("time_bucket").first().reset_index()

# Keep client_id column if exists
if "client_id" not in locations343_buckets.columns:
    locations343_buckets["client_id"] = locations343["client_id"].iloc[0]

# calculate time spent at each bucket location
locations343_buckets = locations343_buckets.sort_values("datetime")
locations343_buckets["next_time"] = locations343_buckets["datetime"].shift(-1)
locations343_buckets["time_spent"] = (locations343_buckets["next_time"] - locations343_buckets["datetime"]).dt.total_seconds()
locations343_buckets["time_spent"] = locations343_buckets["time_spent"].clip(0, 8*3600)

# project coordinates to meters ( also due to memory kill 9 error)
proj = pyproj.Transformer.from_crs("epsg:4326", "epsg:3857", always_xy=True)
locations343_buckets["x"], locations343_buckets["y"] = proj.transform(locations343_buckets["longitude"].values, locations343_buckets["latitude"].values)

# clustering per month
locations343_buckets["month"] = locations343_buckets["datetime"].dt.to_period("M")
df_list = []

for month, df_month in locations343_buckets.groupby("month"):
...
# combine
locations343_buckets = pd.concat(df_list)
...
locations343_buckets = merge_overlapping_clusters(locations343_buckets, distance_threshold=300)

# Update cluster_uid to use merged version
locations343_buckets["cluster_uid"] = locations343_buckets["cluster_uid_merged"]
locations343_buckets = locations343_buckets.drop(columns=["cluster_uid_merged"])

# find centroids of the clusters (after merging)

centroids = (
    locations343_buckets[locations343_buckets["cluster_uid"].str.contains("_-1") == False]
    .groupby("cluster_uid")[["latitude", "longitude"]]
    .mean()
    .reset_index()
)
...
monthly = (
    locations343_buckets[locations343_buckets["cluster_id"] != -1]
...
locations343_buckets["hour"] = locations343_buckets["datetime"].dt.hour
locations343_buckets["weekday"] = locations343_buckets["datetime"].dt.weekday  # 0=Mon ... 6=Sun
...
locations343_buckets["time_category"] = locations343_buckets.apply(categorize, axis=1)

valid = locations343_buckets[locations343_buckets["cluster_id"] != -1]
...
df_heat = locations343_buckets[locations343_buckets["cluster_id"] != -1].copy()
...
df_seq = locations343_buckets.sort_values("datetime")
...
m2.save("./maps/heatmap343.html")
print("heatmap saved → heatmap343.html")